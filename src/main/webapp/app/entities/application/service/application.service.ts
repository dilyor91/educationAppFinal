...
type RestOf<T extends IApplication | NewApplication> = Omit<T, 'birthday' | 'certificateGivenDate'> & {
...
export class ApplicationService {
  ...
  protected convertDateFromClient<T extends IApplication | NewApplication | PartialUpdateApplication>(application: T): RestOf<T> {
    return {
      ...application,
      birthday: application.birthday?.toJSON() ?? null,
      certificateGivenDate: application.certificateGivenDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restApplication: RestApplication): IApplication {
    return {
      ...restApplication,
      birthday: restApplication.birthday ? dayjs(restApplication.birthday) : undefined,
      certificateGivenDate: restApplication.certificateGivenDate ? dayjs(restApplication.certificateGivenDate) : undefined,
    };
  }
